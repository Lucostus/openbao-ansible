...
Staged bootstrap-to-Agent test from clean volumes:

```bash
lab/scripts/openbao-lab-reset.sh --all --build
lab/scripts/openbao-lab-playbook.sh site \
  -e openbao_certificate_mode=bootstrap \
  -e openbao_bootstrap_tls_source=controller
```

The first run initializes OpenBao and keeps the supplied bootstrap listener
certificates without starting `openbao-agent`. Then switch to the lab default
Agent mode:
...
The second run creates per-node Agent AppRole artifacts under
`secure-artifacts/lab/openbao/`, starts `openbao-agent`, and validates
Agent-issued listener certificates for `bao.lab.local` and each node FQDN.
